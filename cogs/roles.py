...
# ---------------------------------------------------
#  PERMISSION HELPERS (same as in moderation.py)
# ---------------------------------------------------
...
def require(level):
    """Decorator to check if the user has at least the specified level."""
    async def predicate(ctx):
        ...
        if levels.get(user_level, 0) >= levels.get(level, 0):
            return True
        embed = discord.Embed(title="❌ Error", description=f"You need at least **{level}** permissions.", color=0xe74c3c)
        if isinstance(ctx, discord.Interaction):
...
class Roles(commands.Cog):
    ...
    async def createrole_logic(self, ctx, name: str):
        ...
        await self.send(ctx, make_embed(ctx, "✅ Role Created", f"Role **{name}** has been created.", 0x2ecc71))
    ...
    async def deleterole_logic(self, ctx, role: discord.Role):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot manage a role higher than or equal to mine.", 0xe74c3c))
        await role.delete()
        await self.send(ctx, make_embed(ctx, "🗑️ Role Deleted", f"Role **{role.name}** has been deleted.", 0xe67e22))
    ...
    # ── Give Role (admin only) ───────
    async def giverole_logic(self, ctx, member: discord.Member, role: discord.Role):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot give a role higher than mine.", 0xe74c3c))
        await member.add_roles(role)
        await self.send(ctx, make_embed(ctx, "✅ Role Given", f"**{role.name}** given to {member.mention}.", 0x2ecc71))
    ...
    @app_commands.command(name="giverole")
    @app_commands.default_permissions(manage_roles=True)
    ...
    async def removerole_logic(self, ctx, member: discord.Member, role: discord.Role):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot remove a role higher than mine.", 0xe74c3c))
        await member.remove_roles(role)
        await self.send(ctx, make_embed(ctx, "✅ Role Removed", f"**{role.name}** removed from {member.mention}.", 0xe67e22))
    ...
    async def renamerole_logic(self, ctx, role: discord.Role, new_name: str):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot rename a role higher than mine.", 0xe74c3c))
        old = role.name
        await role.edit(name=new_name)
        await self.send(ctx, make_embed(ctx, "🏷️ Role Renamed", f"**{old}** → **{new_name}**", 0x3498db))

    @commands.command(aliases=["rnrole"])
    @require("admin")
    async def renamerole(self, ctx, role: discord.Role, *, new_name: str):
        await self.renamerole_logic(ctx, role, new_name)

    @app_commands.command(name="renamerole")
    @app_commands.default_permissions(manage_roles=True)
    ...
    # ── Change Role Color (admin only) ──
    async def rolecolor_logic(self, ctx, role: discord.Role, color: str):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot edit a role higher than mine.", 0xe74c3c))
        try:
            col = int(color.strip("#"), 16)
            col = discord.Color(col)
        except:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "Invalid color. Use hex like `#ff0000` or `ff0000`.", 0xe74c3c))
        await role.edit(color=col)
        await self.send(ctx, make_embed(ctx, "🎨 Role Color Updated", f"Color of **{role.name}** changed to {color}.", 0x9b59b6))

    @commands.command(aliases=["rolecolor", "crcolor"])
    @require("admin")
    async def colorrole(self, ctx, role: discord.Role, *, color: str):
        await self.rolecolor_logic(ctx, role, color)

    @app_commands.command(name="rolecolor")
    @app_commands.default_permissions(manage_roles=True)
    ...
    # ── Move Role Position (admin only) ──
    async def roleposition_logic(self, ctx, role: discord.Role, position: int):
        if role >= ctx.guild.me.top_role:
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I cannot move a role higher than mine.", 0xe74c3c))
        if role.is_default():
            return await self.send(ctx, make_embed(ctx, "❌ Error", "Cannot move the @everyone role.", 0xe74c3c))

        # Convert "top is 1" to Discord's internal system
        ...
        if position < 1 or position > total:
            return await self.send(ctx, make_embed(ctx, "❌ Error", f"Position must be between 1 and {total}.", 0xe74c3c))
        ...
        try:
            ...
            return await self.send(ctx, make_embed(ctx, "❌ Error", "I lack permission to reorder roles.", 0xe74c3c))
        except Exception as e:
            return await self.send(ctx, make_embed(ctx, "❌ Error", f"Failed: {e}", 0xe74c3c))

        await self.send(ctx, make_embed(ctx, "🔄 Role Moved", f"Moved **{role.name}** to position #{position}.", 0x2ecc71))

    @commands.command(aliases=["rpos", "move"])
    @require("admin")
    async def roleposition(self, ctx, role: discord.Role, position: int):
        await self.roleposition_logic(ctx, role, position)

    @app_commands.command(name="roleposition")
    @app_commands.default_permissions(manage_roles=True)
    ...
    async def rolehoist_logic(self, ctx, role: discord.Role, state: bool = None):
        ...
        await self.send(ctx, make_embed(ctx, "🔝 Hoist Toggled", f"**{role.name}** hoist set to **{state}**.", 0x2ecc71))
    ...
    @app_commands.command(name="rolehoist")
    @app_commands.default_permissions(manage_roles=True)
    ...
    async def rolementionable_logic(self, ctx, role: discord.Role, state: bool = None):
        ...
        await self.send(ctx, make_embed(ctx, "📢 Mentionable Toggled", f"**{role.name}** mentionable set to **{state}**.", 0x2ecc71))

    @commands.command(aliases=["mentionable"])
    @require("admin")
    async def rolementionable(self, ctx, role: discord.Role, state: bool = None):
        await self.rolementionable_logic(ctx, role, state)

    @app_commands.command(name="rolementionable")
    @app_commands.default_permissions(manage_roles=True)
